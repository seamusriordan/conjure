...
local function get_scoped_symbols_at_cursor(query)
  ...
  for id, n, meta in captures do
    ...
    elseif (("local.bind" == captured_label) and is_in_scope(cursor_node, get_nth_scope_parent(1, n, scopes))) then
      table.insert(results, get_node_text(n, buffer, meta))
    elseif (("local.define" == captured_label) and is_in_scope(cursor_node, get_nth_scope_parent(2, n, scopes))) then
      table.insert(results, get_node_text(n, buffer, meta))
    else
    end
  end
